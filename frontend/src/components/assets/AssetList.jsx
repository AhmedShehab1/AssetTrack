...
const AssetList = () => {
  ...
  const fetchAssets = useCallback(async (overrides = {}) => {
    ...
    try {
      ...
      // Client-side filtering for 'Allocated To'
      if (activeFilters.allocatedTo) {
        content = content.filter(a => a.currentOwner?.id === activeFilters.allocatedTo);
      }

      // Search fallback
      if (activeFilters.search && content.length === 0 && activePage === 0) {
  ...
  }, [filters, page, availableBrands.length]);
  ...
  return (
    <div className="space-y-8 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-text-heading tracking-tight m-0">Asset Inventory</h2>
          <p className="text-text-body text-sm mt-1 font-medium opacity-80">Unified hardware lifecycle management and tracking.</p>
        </div>
      ...
      <Card padding="p-6" className="bg-slate-50/50 border-outline-variant">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="flex flex-col space-y-1.5 w-full">
            ...
            <div className="relative">
              <select name="allocatedTo" value={filters.allocatedTo} onChange={handleFilterChange} disabled={currentUser?.role === 'DEVELOPER'} className="w-full bg-white border border-gray-200 rounded-xl py-2.5 px-4 text-sm font-bold text-text-heading focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary appearance-none transition-all disabled:opacity-50">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant relative font-sans">
              {loading && assets.length === 0 ? (
                <tr><td colSpan="6" className="py-32 text-center text-text-body font-black uppercase tracking-[0.3em] opacity-30 animate-pulse">Syncing Inventory...</td></tr>
              ) : assets.length === 0 ? (
                <tr><td colSpan="6" className="py-32 text-center text-text-body font-medium italic opacity-60">No hardware found matching your criteria.</td></tr>
              ) : (
                assets.map((asset) => (
                          ...
                          <div className="p-2 space-y-1">
                            ...
                            {currentUser?.role === 'ADMIN' && (
                              <button onClick={() => { setSelectedAsset(asset); openModal('EDIT_ASSET', asset); }} className="w-full flex items-center gap-3 px-3 py-2.5 text-[11px] font-bold text-text-body hover:bg-slate-50 hover:text-primary rounded-xl transition-colors">
                                <Edit3 size={16} /> Edit Asset Profile
                              </button>
                            )}
                            {(currentUser?.role === 'ADMIN' || currentUser?.role === 'MANAGER') && (asset.status === 'AVAILABLE' || asset.status === 'SPARE') && (
