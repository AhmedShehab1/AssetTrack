...
class AllocationServiceTest {
        ...
        class GetAllocationHistory {
                ...
                void success() {
                        ...
                        when(assetRepository.findById(testId)).thenReturn(Optional.of(asset));
                        when(allocationRepository.findByAssetId(eq(testId), any(Pageable.class)))
                                        .thenReturn(new PageImpl<>(List.of(allocation)));
                        when(allocationMapper.toHistoryDto(any(AssetAllocation.class)))
                                        .thenReturn(dto);

                        Page<AllocationHistoryDto> result = allocationService.getAllocationHistory(testId, Pageable.unpaged());

                        assertThat(result.getContent()).hasSize(1);
                        assertThat(result.getContent().get(0).getAllocationId()).isEqualTo(testId);
                }
                ...
                void empty() {
                        ...
                        assertThat(allocationService.getAllocationHistory(testId, Pageable.unpaged()).getContent()).isEmpty();
                }
                ...
                void assetNotFound() {
                        ...
                        assertThatThrownBy(() -> allocationService.getAllocationHistory(testId, Pageable.unpaged()))
                                        .isInstanceOf(ResourceNotFoundException.class);
                }
        }
}