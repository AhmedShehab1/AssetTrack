...
import com.assettrack.service.allocation.AllocationService;
...
public class AllocationController {
    ...
    @PostMapping("")
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<AllocationResponseDto> allocate(@RequestBody @Validated AllocationRequestDto requestDto){
        return ResponseEntity.status(HttpStatus.CREATED).body(allocationService.allocate(requestDto));
    }

    @GetMapping("/{allocationId}")
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<AllocationResponseDto> getAllocation(@PathVariable UUID assetId,@PathVariable UUID allocationId){
