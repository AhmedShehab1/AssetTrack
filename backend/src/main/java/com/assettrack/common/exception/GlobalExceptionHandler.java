...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleHttpMessageNotReadable(HttpMessageNotReadableException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(buildApiError(HttpStatus.BAD_REQUEST, "Malformed JSON request", request.getRequestURI()));
    }
    ...
    public ResponseEntity<ApiError> handleBadCredentials(BadCredentialsException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(buildApiError(HttpStatus.UNAUTHORIZED, "Invalid email or password", request.getRequestURI()));
    }
    ...
    public ResponseEntity<ApiError> handleMethodNotSupported(HttpRequestMethodNotSupportedException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
                .body(buildApiError(HttpStatus.METHOD_NOT_ALLOWED, ex.getMessage(), request.getRequestURI()));
    }
    ...
    public ResponseEntity<ApiError> handleAllExceptions(Exception ex, HttpServletRequest request) {
        if (ex instanceof com.assettrack.exception.BaseException baseException) {
            ...
            return ResponseEntity.status(status)
                    .body(buildApiError(status, baseException.getMessage(), request.getRequestURI()));
        }

        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(buildApiError(HttpStatus.INTERNAL_SERVER_ERROR, "An unexpected error occurred", request.getRequestURI()));
    }
}
